...
/* END OF NAVIGATION DROPDOWN SCROLLING */
...
(function($, window, document, undefined) {
    var Portfolio = {
        init: function(options, elem) {
            ...
            self.$elem = $(elem);
            self.$thumnail = $(elem).find('a.thumbnail');
        ...
        setColWidth: function() {
            ...
            if ($(window).width() > 970) {
                ...
                self.$elem.find('li.thumb').css('width', colWidth + '%');
            }
        },
        getContentPos: function(clicked) {
            var self = this,
                thumbnails = self.$elem.find('a.thumbnail'),
                contentPos = null;
            ...
            for (var i = 0; i <= thumbnails.length; i++) {
                ...
                if (href == $(thumbnails[i]).attr('href')) {
                    ...
                    if (i !== (thumbnails.length - 1)) {
                        ...
                        if ($(window).width() <= 1200 && $(window).width() > 970) cols = 4;
        ...
        showContent: function(thumbnail) {
            ...
            self.$elem.find('li.thumb a.thumbnail .active-arrow').remove();
			...
			var output = '<li class="content"><div class="container">' + $portfolioContent.html() + '</div><span class="close" >Close</span></li>';
			self.$elem.find('li.thumb:eq(' + (contentPos - 1) + ')').after(output);
			$widgetkit.lazyload();
            
			// Animate
            self.$elem.find('li.content')[self.options.transition](500);
			$('html, body').animate({
                scrollTop: self.$elem.find('li.content').offset().top -90
            }, 700);
			

            // Close content
            self.$elem.find('span.close').click(function(){
                self.$elem.find('li.thumb a.thumbnail .active-arrow').remove();
                self.$elem.find('li.content').remove();
            });
        }
